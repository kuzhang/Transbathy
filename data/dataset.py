...
class BathyDataset(Dataset):
    ...
    def __init__(self, config, split='train'):
        ...
        self.data_interval, self.data_ids, self.shp_list, self.data_lons, self.data_lats, data_infos, self.shp_names = self.load_raster_shp()

        # random shuffle shape file
        total_len = self.data_interval[-1] + 1
        train_len, test_len, val_len = [int(i * total_len) for i in self.config['Data']['split']]
        ...
        # self.save_visual_data(shp_dict['train'])
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            print('shp_id:\n',shp_id)
            print('index:{} and index rec:{}\n'.format(index,index_rec))
            print('shp:\n', self.shp_names)
            print('shp:\n', shp)
    ...
    def load_raster_shp(self):
        ...
        return np.array(data_interval), img_ids, shp_list, img_lons, img_lats, data_infos, shp_names
    ...
    # def save_visual_data(self,shp_dict):
    #     observations = []
    #     lons = []
    #     lats = []
    #     for index in shp_dict:
    #         shp_id = np.where((self.data_interval - index) >= 0)[0][0]
    #         if shp_id > 0:
    #             index_rec = index - self.data_interval[shp_id - 1] - 1
    #         else:
    ...
    #             print('shp:\n', shp)
    #         depth = torch.tensor([depth], dtype=torch.float32)
    #         observations.append(depth)
    #
    #         lons.append(shp['Longitude'].iloc[index_rec])
    #         lats.append(shp['Latitude'].iloc[index_rec])
    #
    #     visual_data = {
    #         'lons': lons,
    #         'lats': lats,
